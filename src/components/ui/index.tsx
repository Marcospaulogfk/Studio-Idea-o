...
export function Button({
  ...
  const variants = {
    ...
    secondary: 'bg-white dark:bg-neutral-900 text-gray-700 dark:text-neutral-200 border border-gray-200 dark:border-neutral-800 hover:bg-gray-50 dark:hover:bg-neutral-800/60 shadow-sm',
    ghost:     'text-gray-600 dark:text-neutral-300 hover:bg-gray-100 dark:hover:bg-neutral-900',
...
export function Badge({ children, variant = 'gray', pulse, className }: BadgeProps) {
  const variants = {
    ...
    gray:   'bg-gray-100   text-gray-700    border-gray-200    dark:bg-neutral-800/50   dark:text-neutral-300   dark:border-neutral-700',
...
export function Card({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={cn(
      'bg-white dark:bg-neutral-900 rounded-2xl shadow-sm hover:shadow-md transition-shadow',
      'border border-gray-100 dark:border-neutral-800 p-6',
      className,
    )}>
      {children}
    </div>
  ...
  return (
    <div className="flex items-start justify-between mb-6">
      <div>
        ...
        {subtitle && <p className="text-sm text-gray-500 dark:text-neutral-400 mt-0.5">{subtitle}</p>}
      </div>
      {action}
    </div>
  )
...
const KPI_THEME: Record<KpiColor, { border: string; iconBg: string; gradient: string }> = {
  orange: { border: 'border-l-orange-500', iconBg: 'bg-orange-50 text-orange-600 dark:bg-orange-500/10 dark:text-orange-400', gradient: 'from-white to-orange-50/40 dark:from-neutral-900 dark:to-orange-500/5' },
  blue:   { border: 'border-l-blue-500',   iconBg: 'bg-blue-50   text-blue-600   dark:bg-blue-500/10   dark:text-blue-400',   gradient: 'from-white to-blue-50/40   dark:from-neutral-900 dark:to-blue-500/5'   },
  green:  { border: 'border-l-green-500',  iconBg: 'bg-green-50  text-green-600  dark:bg-green-500/10  dark:text-green-400',  gradient: 'from-white to-green-50/40  dark:from-neutral-900 dark:to-green-500/5'  },
  yellow: { border: 'border-l-yellow-500', iconBg: 'bg-yellow-50 text-yellow-600 dark:bg-yellow-500/10 dark:text-yellow-400', gradient: 'from-white to-yellow-50/40 dark:from-neutral-900 dark:to-yellow-500/5' },
  red:    { border: 'border-l-red-500',    iconBg: 'bg-red-50    text-red-600    dark:bg-red-500/10    dark:text-red-400',    gradient: 'from-white to-red-50/40    dark:from-neutral-900 dark:to-red-500/5'    },
  purple: { border: 'border-l-purple-500', iconBg: 'bg-purple-50 text-purple-600 dark:bg-purple-500/10 dark:text-purple-400', gradient: 'from-white to-purple-50/40 dark:from-neutral-900 dark:to-purple-500/5' },
}
...
export function KpiCard({ title, value, subtitle, icon, trend, color = 'orange' }: KpiCardProps) {
  ...
  return (
    <div className={cn(
      ...
      'border border-gray-100 dark:border-neutral-800 border-l-4 p-5',
      t.border, t.gradient,
    )}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-xs font-medium text-gray-500 dark:text-neutral-400 uppercase tracking-wide">{title}</p>
          <p className="text-2xl font-bold text-gray-900 dark:text-neutral-100 mt-1.5 truncate">{value}</p>
    ...
    <div className="space-y-1">
      ...
      <select
        className={cn(
          ...
          error ? 'border-red-300 dark:border-red-500/30' : 'border-gray-200 dark:border-neutral-800',
          className,
        )}
        {...props}
      >
...
export function EmptyState({ icon, title, description, action }: {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      <div className="w-14 h-14 bg-gray-100 dark:bg-neutral-800/50 rounded-2xl flex items-center justify-center text-gray-400 dark:text-neutral-400 mb-4">
        {icon}
      </div>
      <h3 className="text-base font-semibold text-gray-800 dark:text-neutral-200">{title}</h3>
      {description && <p className="text-sm text-gray-500 dark:text-neutral-400 mt-1 max-w-xs">{description}</p>}
...
export function ProgressBar({ value, max, className }: { value: number; max: number; className?: string }) {
  ...
  return (
    <div className={cn('w-full bg-gray-100 dark:bg-neutral-800 rounded-full h-2 overflow-hidden', className)}>
  ...
  return (
    <div className="relative inline-flex items-center justify-center" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="-rotate-90">
        <circle cx={size/2} cy={size/2} r={radius} strokeWidth={strokeWidth} className="stroke-gray-100 dark:stroke-neutral-800" fill="none" />
        <circle
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <p className="text-2xl font-extrabold text-gray-900 dark:text-neutral-100">{Math.round(pct)}%</p>
        {label && <p className="text-[10px] uppercase tracking-wider text-gray-500 dark:text-neutral-400 mt-0.5">{label}</p>}
      </div>
    </div>
  )
}
